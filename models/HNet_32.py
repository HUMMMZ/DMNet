...
class H_Net_32(nn.Module):
    def __init__(self,  in_ch, out_ch, bn=True, BatchNorm=False):
        super(H_Net_32, self).__init__()
        ...
        self.d1_conv = nn.Conv2d(64, 64, kernel_size=1, padding=0, stride=1, bias=True)
    ...
    def forward(self, x):
        ...
        x_3 = F.upsample(l_x, size=(int(img_shape / 4), int(img_shape / 4)), mode='bilinear')
        x_4 = F.upsample(l_x, size=(int(img_shape / 8), int(img_shape / 8)), mode='bilinear')
        conv1, out1 = self.down1(l_x) # torch.Size([1, 32, 128, 128])
        out1 = torch.cat([self.conv2(x_2), out1], dim=1) #torch.Size([1, 96, 128, 128])
        conv2, out2 = self.down2(out1) # torch.Size([1, 64, 64, 64])
        out2 = torch.cat([self.conv3(x_3), out2], dim=1) # torch.Size([1, 192, 64, 64])
        conv3, out3 = self.down3(out2)  # torch.Size([1, 128, 32, 32])
        out3 = torch.cat([self.conv4(x_4), out3], dim=1) #torch.Size([1, 384, 32, 32])
        conv4, out4 = self.down4(out3) # torch.Size([1, 256, 16, 16])
        # cet_out = self.center(out)
        # CAC_out = self.CAC(out4)
...
class H_Net_139(nn.Module):
    def __init__(self,  in_ch, out_ch, bn=True, BatchNorm=False):
        ...
        #
        self.d3_conv = nn.Conv2d(128, 128, kernel_size=1, padding=0, stride=1, bias=True)
        self.d2_conv = nn.Conv2d(64, 64, kernel_size=1, padding=0, stride=1, bias=True)
        self.d1_conv = nn.Conv2d(64, 64, kernel_size=1, padding=0, stride=1, bias=True)
